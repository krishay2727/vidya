...
// Sensitivity multiplier for BLE HID mouse mode (can be modified over serial)
float sensitivity = 1.0f;

// Bluetooth HID Configuration
BleMouse bleMouse("ESP32 Air Mouse", "Tinkering Lab", 100);
...
const unsigned long debounceDelay = 30; // milliseconds
...
void loop() {
  // Update BLE mouse connection status variable
  bleMouseConnected = bleMouse.isConnected();
  ...
  if (currentLeft != lastLeftState) {
    if ((millis() - lastLeftDebounce) > debounceDelay) {
      ...
      if (currentLeft == LOW) { // Button pressed
        if (bleMouseConnected) {
          bleMouse.press(MOUSE_LEFT);
        }
        Serial.println("{\"status\":\"button\",\"message\":\"Left click pressed\"}");
      } else { // Button released
        if (bleMouseConnected) {
          bleMouse.release(MOUSE_LEFT);
        }
        Serial.println("{\"status\":\"button\",\"message\":\"Left click released\"}");
      }
      lastLeftState = currentLeft;
    }
  }

  // Handle Right Button Click/Drag
  ...
  if (currentRight != lastRightState) {
    if ((millis() - lastRightDebounce) > debounceDelay) {
      ...
      if (currentRight == LOW) { // Button pressed
        if (bleMouseConnected) {
          bleMouse.press(MOUSE_RIGHT);
        }
        Serial.println("{\"status\":\"button\",\"message\":\"Right click pressed\"}");
      } else { // Button released
        if (bleMouseConnected) {
          bleMouse.release(MOUSE_RIGHT);
        }
        Serial.println("{\"status\":\"button\",\"message\":\"Right click released\"}");
      }
  ...
  if (readRegisters(ACCEL_XOUT_H_REG, buf, 14)) {
    ...
    Serial.print(bleMouseConnected ? "true" : "false");
    Serial.println("}");
    
    // Map gyro rates to cursor movement
    // Z axis controls Yaw (Horizontal X motion)
    // Y axis controls Pitch (Vertical Y motion)
    // Apply deadzone and scaling factor
    float deadzone = 0.5f; // ignore gyro noise below 0.5 deg/s
    int8_t mouseX = 0;
    int8_t mouseY = 0;

    if (abs(gz_ds) > deadzone) {
      // Sensitivity factor controls the speed. Default 1.0f.
      // -gz_ds is horizontal movement (negating depends on preferred direction/sensor mount)
      mouseX = (int8_t)(-gz_ds * sensitivity * 0.4f);
    }
    if (abs(gy_ds) > deadzone) {
      // -gy_ds controls vertical movement
      mouseY = (int8_t)(-gy_ds * sensitivity * 0.4f);
    }
